...
export const AshesModal = ({ open, handleClose }: IModalProps) => {
  ...
  const handleSave = () => {
    setValue("body_buried_cremated_primary", textFieldValue);
    trigger("body_buried_cremated_primary");
    handleClose();
  };
  return (
    <>
...
export const LaidModal = ({ open, handleClose }: IModalProps) => {
  const { setValue, trigger } = useFormContext();
  const [copy, setCopy] = useState(false);
  const [copyIndex, setCopyIndex] = useState<null | number>(null);
  const [textFieldValue, setTextFieldValue] = useState("");
          ...
          >
            ...
            {laidSuggestion?.map((el, i) => (
              ...
              >
                ...
                {i === copyIndex && copy ? (
                  <Done sx={{ fontSize: "16px", ml: 1, color: "black" }} />
                ) : (
                  <Button
                    variant="contained"
                    sx={{
...
export const FuneralModal = ({ open, handleClose }: IModalProps) => {
  const { setValue, trigger } = useFormContext();
  const [copy, setCopy] = useState(false);
  const [copyIndex, setCopyIndex] = useState<null | number>(null);
  const [textFieldValue, setTextFieldValue] = useState("");
  ...
  const handleSave = () => {
    setValue("funeral_service_primary", textFieldValue);
    trigger("funeral_service_primary");
    handleClose();
  };
  return (
    <>
...
export const ArrangementModal = ({ open, handleClose }: IModalProps) => {
  const { setValue, trigger } = useFormContext();
  const [copy, setCopy] = useState(false);
  const [copyIndex, setCopyIndex] = useState<null | number>(null);
  const [textFieldValue, setTextFieldValue] = useState("");
  ...
  const handleSave = () => {
    setValue("post_death_arrangement_primary", textFieldValue);
    trigger("post_death_arrangement_primary");
    handleClose();
  };
  return (
    <>
