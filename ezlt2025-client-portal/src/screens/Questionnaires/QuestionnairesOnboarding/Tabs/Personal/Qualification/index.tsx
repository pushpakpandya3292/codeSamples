...
import { useClientCache, useDeleteClientCache } from "@/provider/ClientCache";
...
const Qualification: React.FC<QualificationProps> = () => {
  ...
  const { mutateAsync: deleteCacheMutate, isLoading } = useDeleteClientCache();
  const router = useRouter();
  const [showModal, setShowModal] = useState(false);
  const handleClose = () => {
    setShowModal(false);
  };
  const handlDeleteDocument = async () => {
    const data = await deleteCacheMutate({
      id: sessionStorage.getItem("interviewCacheId"),
    });
    sessionStorage.removeItem("interviewCacheId");
    router.push("/dashboards/living-trust-interview");
  };

  useEffect(() => {
    if (watch("is_qualified") === "no") {
      setShowModal(true);
    }
  }, [watch("is_qualified")]);
  return (
    <>
      ...
      <CustomModal width={{ xs: "100%", sm: "558px" }} height={"auto"} open={showModal} handleClose={handleClose}>
        <Box sx={{ textAlign: "center" }}>
          ...
          >
            <Button sx={{ width: { xs: "100%", md: "auto" } }} onClick={handleClose} variant="contained">
              Review my answers
            </Button>
            {/* <Button
              variant="contained"
            ...
            <Button
              sx={{ ml: { xs: 0, md: 2 }, backgroundColor: "#F00", width: { xs: "100%", md: "auto" } }}
              variant="contained"
              onClick={handlDeleteDocument}
            >
              {isLoading ? "Deleting..." : "Delete this document"}
            </Button>
          </Box>
        </Box>
      </CustomModal>
