...
import PrimaryTrustee from "./Tabs/Personal/PrimaryTrustee";
...
} from "@/constants/Validations/ValidatorSingle";
...
import LifeInsurance from "./Tabs/Services/LifeInsurance";
...
import { formatDataSingle } from "../dataMiddleWare";
...
const QuestionnairesSingle: React.FC<Props> = ({ cartId }) => {
  ...
  useEffect(() => {
    if (cacheData.isSuccess && cacheData?.data?.data) {
      ...
      setInterViewFormValues({
        ...
        //@ts-ignore
        died_person: cacheData?.data?.data.died_person,
        last_married_person: cacheData?.data?.data.last_married_person,
        cartId: cartId,
          ...
          >
            <Box>
              {!(activeStep === 0 && activeTab === 0) && (
                <Button
                  variant="contained"
                  sx={{ px: { xs: 1, md: 5 }, py: 1, textTransform: "none" }}
      ...
      >
        ...
        >
          <Pageheader
            ...
            // active="Individual Living Trust"
          />
          <Box
            sx={{
              width: "100%",
...
export default QuestionnairesSingle;
